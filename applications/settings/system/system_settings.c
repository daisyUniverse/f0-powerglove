...
const char* const powerglove_text[] = {
    "OFF",
    "ON",
};
...
const uint32_t powerglove_value[] = {
    LocaleTimeFormat24h,
    LocaleTimeFormat12h,
};
...
static void powerglove_changed(VariableItem* item) {
    uint8_t index = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, time_format_text[index]);
    locale_set_time_format(time_format_value[index]);
}
...
SystemSettings* system_settings_alloc(void) {
    ...
    item = variable_item_list_add(
        app->var_item_list, "POWERGLOVE", COUNT_OF(powerglove_text), powerglove_changed, app);
    value_index = value_index_uint32(
        locale_get_time_format(), powerglove_value, COUNT_OF(powerglove_value));
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, powerglove_text[value_index]);
